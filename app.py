...
def textbox(text, box="AI", name="Philippe", additional_class=""):
    ...
    if box == "user":
        ...
        return dbc.Card(text, style=style, body=True, color="primary", inverse=True, className=additional_class)
        ...
        return html.Div([thumbnail, dbc.Card(text, style=style, body=True, color="light", inverse=False, className=additional_class)])
...
controls = dbc.Row([
    ...
    dbc.Col(
        ...
        style={"flex": "1", "maxWidth": "100px"}  # max width to limit the button width
    )
], style={"display": "flex"})  # Display flex for parent to control children's width
...
@app.callback(
    [Output("store-conversation", "data"), Output("display-conversation", "children"), Output("user-input", "value")],
    [Input("model-choice", "value"), Input("submit", "n_clicks"), Input("user-input", "n_submit")],
    [State("user-input", "value"), State("store-conversation", "data")]
)
def update_conversation(model_choice_value, n_clicks, n_submit, user_input, chat_history):
    ...
    if triggered_id == "model-choice":
        ...
        chat_bubbles = [textbox(message, box="user" if i % 2 == 0 else "AI") for i, message in enumerate(messages)]
